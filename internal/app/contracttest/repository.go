...
func checkRepository(ctx context.Context, repo Repository) {
	...
	session, err := repo.GetSessionByToken(ctx, "contract-token")
	...
	_, err = repo.GetSessionByToken(ctx, "contract-token")
	...
	err = repo.TakeSubRequest(ctx, request.ID, user.ID)
	...
	err = repo.UntakeSubRequest(ctx, request.ID)

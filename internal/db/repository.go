...
func (r *Repository) CreateSubRequest(ctx context.Context, sr *models.SubRequest) error {
	res, err := r.db.ExecContext(ctx, `
		INSERT INTO sub_requests (show_id, posted_by_user_id, start_time, end_time, notes, status, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, sr.ShowID, sr.PostedByUserID, sr.StartTime, sr.EndTime, sr.Notes, sr.Status, sr.CreatedAt, sr.UpdatedAt)
	if err != nil {
		return err
	}
	id, err := res.LastInsertId()
...
func (r *Repository) ListSubRequests(ctx context.Context) ([]*models.SubRequest, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT sr.id, sr.show_id, sr.posted_by_user_id, u.email, sr.start_time, sr.end_time, sr.notes, sr.status, sr.created_at, sr.updated_at
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&sr.ID, &sr.ShowID, &sr.PostedByUserID, &sr.RequesterEmail,
...
func (r *Repository) GetSubRequestByID(ctx context.Context, id int) (*models.SubRequest, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT id, show_id, posted_by_user_id, start_time, end_time, notes, status, created_at, updated_at
		FROM sub_requests
		WHERE id = ?
	`, id).Scan(
		&sr.ID, &sr.ShowID, &sr.PostedByUserID, &sr.StartTime, &sr.EndTime, &sr.Notes, &sr.Status, &sr.CreatedAt, &sr.UpdatedAt,
	)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, ErrNotFound
